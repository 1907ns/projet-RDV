...
struct Sphere {
    ...
    // deathstar ray intersection function
    bool deathstar_ray_intersect(const Vec3f &orig, const Vec3f &dir, float &t1) const {
        Vec3f L = center - orig;
        float tca = L*dir;
        float d2 = L*L - tca*tca;
        if (d2 > radius*radius) return false;
...
bool scene_intersect(const Vec3f &orig, const Vec3f &dir, const std::vector<Sphere> &spheres, Vec3f &hit, Vec3f &N, Material &material) {
    ...
    for (size_t i=0; i < spheres.size()-2; i++) {
        float dist_i;
    ...
    // deathstar
    float deathstar_dist1;
    if (spheres[spheres.size()-1].deathstar_ray_intersect(orig, dir, deathstar_dist1) && deathstar_dist1 < spheres_dist) { 
        Vec3f hitvec1 = orig + dir*deathstar_dist1;
        float Xdist = pow(spheres[spheres.size()-2].center.x - hitvec1.x, 2);
        float Ydist = pow(spheres[spheres.size()-2].center.y - hitvec1.y, 2);
        float Zdist = pow(spheres[spheres.size()-2].center.z - hitvec1.z, 2);
        if (sqrt(Xdist + Ydist + Zdist) <= spheres[spheres.size()-2].radius){
            spheres_dist = deathstar_dist1;
            hit = hitvec1;
            N = (-(hit - spheres[spheres.size()-1].center)).normalize();
            material = spheres[spheres.size()-1].material;
        }
    }

    float deathstar_dist2;
    if (spheres[spheres.size()-2].ray_intersect(orig, dir, deathstar_dist2) && deathstar_dist2 < spheres_dist){ 
        Vec3f hitvec2 = orig + dir*deathstar_dist2;
        float Xdist = pow(spheres[spheres.size()-1].center.x - hitvec2.x, 2);
        float Ydist =  pow(spheres[spheres.size()-1].center.y - hitvec2.y, 2);
        float Zdist = pow(spheres[spheres.size()-1].center.z - hitvec2.z, 2);
        if (sqrt( Xdist + Ydist + Zdist ) >= spheres[spheres.size()-1].radius){
            spheres_dist = deathstar_dist2;
            hit = hitvec2;
            N = (hit - spheres[spheres.size()-2].center).normalize();
            material = spheres[spheres.size()-2].material;
        }
    }
...
int main() {
    ...
    // spheres which are concerned by deathstar
    spheres.push_back(Sphere(Vec3f( -8,    5,   -18), 4.5,     red_rubber)); // big sphere
    spheres.push_back(Sphere(Vec3f( -5,    5,   -15), 2,     ivory)); // small sphere
